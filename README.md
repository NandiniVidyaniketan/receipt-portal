            ...
            <table class="receipt-fee-table">
                ...
                <tfoot>
                    <tr class="total-row">
                        ...
                        <td><strong>₹ <span id="receiptTotal">0.00</span></strong></td>
                    </tr>
                </tfoot>
            </table>
 ...
 */
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbywo08UtyklgHjhWVzX_gPn9aufXK92WQd7BWZi57bBdYM3mzbSv5J7mtPIF5QSh1wY7w/exec';
...
function fetchNextReceiptNumber() {
    console.log('📋 Fetching next receipt number from Google Sheets...');
    ...
    fetch(GOOGLE_SCRIPT_URL + '?action=getNextReceipt')
        ...
        .then(data => {
            ...
            if (data.status === 'success' && data.nextReceiptNumber) {
                ...
                console.log('✅ Next receipt number:', data.nextReceiptNumber);
            } else {
                // Fallback to NVA001 if something goes wrong
                receiptNumberField.value = 'NVA001';
                console.warn('⚠️ Could not fetch receipt number, using NVA001');
            }
        })
        .catch(error => {
...
function sendDataToGoogleSheets() {
    ...
    if (GOOGLE_SCRIPT_URL === 'PASTE_YOUR_WEB_APP_URL_HERE') {
        console.error('❌ Google Apps Script URL not configured!');
        showNotification('⚠️ Google Sheets integration not configured', 'error');
        return;
    }
    ...
    .then(text => {
        ...
        try {
            ...
            showNotification('✅ Receipt sent to Google Sheets', 'success');
            setTimeout(() => {
                fetchNextReceiptNumber();
            }, 1000);
        }
